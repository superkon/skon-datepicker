...
sk_datePicker.prototype.resetTag = function(){
  var _self = this;

  $('.weekdayItem.wEvent.selected',_self.target).removeClass('selected');
  $('.displayDateText', _self.target).remove();
};
    ...
    if (_counter === 0) {
        ...
        $('.weekdayItem', _self.target).off('click').on('click', function() {
            ...
            if(($(this).hasClass('wEvent'))&&(!$(this).hasClass('selected')))
            {
                ...
                {
                    _htmlContent += '<a href="javascript:void(0);" class="timeItem" data-id="'+_self.listInfo.listArray[0][_date].time[_i][0]+'"><span class="text">'+_self.listInfo.listArray[0][_date].time[_i][1]+'</span></a>';
                }
